...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/08/08 15:48:00 by msakovyc          #+#    #+#             */
/*   Updated: 2018/08/08 15:48:02 by msakovyc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	putpix(t_window *wnd, int x, int y)
{
	...
	if (x != wnd->x - 1 && (m = -1))
		while (change(x, 'x', wnd) + m++ < change(x + 1, 'x', wnd))
			mlx_pixel_put(wnd->mlx_ptr, wnd->win_ptr,
				change(x, 'x', wnd) + m, change(y, 'y', wnd), 0xffffff);
	if (y != wnd->y - 1 && (n = -1))
		while (change(y, 'y', wnd) + n++ < change(y + 1, 'y', wnd))
			mlx_pixel_put(wnd->mlx_ptr, wnd->win_ptr,
				change(x, 'x', wnd), change(y, 'y', wnd) + n, 0xffffff);
}
...
int		draw(t_window *wnd)
{
	...
	if (wnd->result)
	{
		mlx_string_put(wnd->mlx_ptr, wnd->win_ptr, 10, 10, 0xb3ffd9,
			wnd->result1);
		mlx_string_put(wnd->mlx_ptr, wnd->win_ptr, 10, 30, 0xff9999,
			wnd->result2);
	}
